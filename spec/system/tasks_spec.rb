...
RSpec.describe 'タスク管理機能', type: :system do
  ...
  describe '一覧表示機能' do
    let!(:task) { FactoryBot.create(:task, title: 'first_task', created_at: '2022-02-18') }
    let!(:task) { FactoryBot.create(:task, title: 'second_task', created_at: '2022-02-17') }
    let!(:task) { FactoryBot.create(:task, title: 'third_task', created_at: '2022-02-16') }
    before do
      visit tasks_path
    end
    ...
    context '新たにタスクを作成した場合' do
      it '新しいタスクが一番上に表示される' do
        ...
        visit tasks_path
        task_list = all('tbody tr')
        expect(task_list[0]).to have_content 'aaa作成'
      end
    end
  end
